...
/// Gate parity with `ClientState::start()`: with the local-override master
/// switch off, preview skips the custom outbound / experimental slices while
/// still injecting the DNS slice.
#[tokio::test]
async fn preview_core_config_gates_slices_by_local_override_master_switch() {
    let dir = TestDir::new();
    let cfg = ClientConfig::new(
        dir.path().to_path_buf(),
        String::new(),
    ...
    // Enabled slices: DNS (required) + custom outbound + experimental.
    let slices: ConfigSlices = serde_json::from_value(serde_json::json!({
        "version": 1,
        "dns": {
            "enabled": true,
            "mode": "takeover",
    ...
    // DNS slice still injected.
    assert!(
        value["dns"]["servers"]
            .as_array()
            .unwrap()
            .iter()
            .any(|s| s["tag"] == "slice-dns"),
        "preview must keep the DNS slice when the master switch is off"
    );
    // Outbound slice skipped.
    assert!(
        value["outbounds"]
            .as_array()
            .unwrap()
        ...
        "preview must skip the outbound slice when the master switch is off"
    );
    // Experimental slice skipped.
    assert!(
        value["experimental"].get("cache_file").is_none(),
        "preview must skip the experimental slice when the master switch is off"
    );
}
