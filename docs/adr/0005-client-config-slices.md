...
- **内置规则/分组物化模型（2026-09 补记）**：内置 CN 分流规则与 proxy/auto 分组不再是
  ...
  字段覆写而非追加出站（成员列表动态计算跟随订阅）。；同月再调整为**内置普通化**终态：内置规则集仅保留私有域名/私有 IP 并物化为
  `custom_rule_sets` 普通条目（可编辑/删除/市场重加，`builtins_seeded` 墓碑保证只播种
  一次），国家类规则集退役为市场自选；内置路由规则仅保留「私有直连」一条（普通规则，
  可改可删可还原）；引用未定义规则集的规则在物化阶段剥离，不再阻断核心启动。出站侧新
  增 `global`（含其它内置出站，全局模式改走它）与 `final`（节点选择 + 直连，
  `route.final` 改指它）两个内置 selector；`proxy`/`auto` 成员随订阅动态计算，
  `global`/`final` 静态成员可编辑（防自引用与 global↔final 循环）
