...
/// 读取 toast 渲染模式环境变量覆盖（`PP_TOAST_MODE`）：`static`（兼容 `safe`）
/// 强制禁用 HeroUI 原生 toast（走自实现静态路径，供 WSL/WebKitGTK 等特殊环境
/// 规避 view-transition SIGSEGV）；其余值 / 未设置返回 None（默认启用 HeroUI
/// 原生 toast）。
#[tauri::command]
