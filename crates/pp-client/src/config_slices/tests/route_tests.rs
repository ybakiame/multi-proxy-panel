...
#[test]
fn route_default_has_no_overrides() {
...
fn route_missing_fields_fall_back_to_defaults() {
    ...
    let empty: RouteSlice = serde_json::from_str(r#"{"resolver":{}}"#).unwrap();
...
#[test]
fn validate_accepts_non_empty_slice() {
    let slices = slices_with_route(RouteSlice {
...
#[test]
fn apply_empty_slice_leaves_config_unchanged() {
    let mut config = json!({ "route": { "final": "proxy" } });
    let original = config.clone();
    let slices = slices_with_route(RouteSlice::default());
    apply_config_slices(&mut config, &slices).unwrap();
